...
export const canManageUsers: RoleCheck = (user) =>
  user?.role === 'SUPERADMIN' || user?.role === 'COORDINATION' || user?.role === 'MANAGER';
...
export const canManageSectors: RoleCheck = (user) =>
...
export const canDeleteExternalPartners: RoleCheck = (user) =>
  user?.role === 'SUPERADMIN' || user?.role === 'MANAGER';
...
export const canApproveExchanges: RoleCheck = (user) =>
  user?.role === 'SUPERADMIN' || user?.role === 'MANAGER';
...
export const canViewAudit: RoleCheck = (user) =>
  user?.role === 'SUPERADMIN' || user?.role === 'MANAGER' || user?.role === 'COORDINATION';

export const canExportAudit: RoleCheck = (user) =>
  user?.role === 'SUPERADMIN' || user?.role === 'MANAGER';

// ============ ADMIN ============

/** Acesso à zona de administração técnica (exclusivo SUPERADMIN). */
export const canViewAdmin: RoleCheck = (user) => user?.role === 'SUPERADMIN';

export const canManageSettings: RoleCheck = canViewAdmin;
export const canImpersonate: RoleCheck = canViewAdmin;
export const canRunCleanup: RoleCheck = canViewAdmin;

// ============ HELPERS GLOBAIS ============

/** Usuário vê todos os setores (sem escopo de setor). */
export const hasGlobalView: RoleCheck = (user) =>
  user?.role === 'SUPERADMIN' || user?.role === 'MANAGER';
